...
err_FR = []
FR.append(F1-FD1)
err_FR.append(err_F1+err_F1)
FR.append(F5-FD2)
err_FR.append(err_F5+err_F5)
FR.append(F5-FD3)
err_FR.append(err_F5+err_FD3)
...
for i in range(countFiles):
    ...
    x_data.append([(30*j) for j in range(s[i])])
    xerr.append([err_t for j in range(s[i])])
    y_data.append(beamData[i])
    yerr.append([err_T for j in range(s[i])])
