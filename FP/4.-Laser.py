...
for i in range(countFiles):
    ...
    x_data_single.pop(n)
    y_data_single.pop(n)
    x_data_single = (fitRes[1][0][0] * np.array(x_data_single)) + fitRes[1][0][1]
    s.append(len(x_data_single))
    x_data.append(x_data_single)
    xerr.append([(fitRes[1][0][0] * U_x_err) + (perr[1][0] * x_data_single[j]) + perr[1][1] for j in range(s[i])])
...
for i in range(1):
    ...
    b.append(np.around([perr[j][1] for j in range(len(d))],3))
    b.append(np.around([fitRes[j][0][2] for j in range(len(d))],2))
    b.append(np.around([perr[j][2] for j in range(len(d))],3))
    labels = ["$d$ in cm", "$I_0$ in V", "$\Delta I_0$ in V", "$w$ in cm", "$\Delta w$ in cm", "$x_0$ in cm", "$\Delta x_0$ in cm"]
    b = np.array(b).T        
    fig.append(plt.figure())
    ax.append(fig[i].add_axes([0,0,1,1]))
    ax[i].table(cellText=b,colLabels=labels,loc='center',rowLoc='center')
...
for i in range(1):
    ...
    b.append(np.around(theta,5))
    b.append(np.around(theta_err,5))
    labels = ["$d$ in cm", "$a$ in cm", "$\Theta$ in ˚", "$\Delta \Theta$ in ˚"]
    b = np.array(b).T        
    fig.append(plt.figure())
    ax.append(fig[i].add_axes([0,0,1,1]))
    ax[i].table(cellText=b,colLabels=labels,loc='center',rowLoc='center')
